...
function createResource($db, $data) {
    ...
    if (empty($data['title']) || empty($data['link'])) {
        sendResponse(['success' => false, 'message' => 'Title and link are required fields.'], 400);
        return;
    }
...
function createComment($db, $data) {
    ...
    if (empty($data['resource_id']) || empty($data['author']) || empty($data['text'])) {
        sendResponse(['success' => false, 'message' => 'Resource ID, author, and text are required fields.'], 400);
        return;
    }
